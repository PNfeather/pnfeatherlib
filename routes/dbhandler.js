...
//find
var find = function(db,collections,selector,fn,sortType){
...
var methodType = {
    ...
    deleteExperience: deletes,
    editCollection: updates,
    editExperience: updates,
    page: page //分页
};
//主逻辑    服务器  ， 请求    --》
// req.route.path ==》 防止前端的请求 直接操作你的数据库
module.exports = function(req,res,collections,selector,fn,sortType){
    MongoClient.connect(Urls, function(err, db) {
        assert.equal(null, err);
        console.log("Connected correctly to server");
        // 根据 请求的地址来确定是什么操作  （为了安全，避免前端直接通过请求url操作数据库）
        methodType[req.route.path.substr(1)](db,collections,selector,fn, sortType);
